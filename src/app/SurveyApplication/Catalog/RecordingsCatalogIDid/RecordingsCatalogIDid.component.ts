...
@Component({
  selector: 'app-Recordings-catalogIDid',
  templateUrl: './RecordingsCatalogIDid.component.html',
  styleUrls: ['./RecordingsCatalogIDid.component.css']
})
export class RecordingsCatalogIDidComponent implements OnInit {
  ...
  GetRecordingListByUserID(uid) {
    if (uid) {
      ...
      recordingObj.UserID = uid;
      
      return this.http
      // .post(`${environment.domainApi}I2BRecordingRequest/GetRequestedFromMe`, recordingObj).subscribe(res=>{
      .get(`https://linappcardtest.azurewebsites.net/api/getallrecordingsfromupload1`).subscribe(res=>{
        ...
        /*if (res["length"] > 0) {
          ...
          if (res[0]['ResponseStatus'] == 'Success') {
            ...
            //let data_filter = this.recordingList.filter(element => (element.RecordingStatus.trim() == 'Finished' && element.Convertedflag.trim() == 'True'))
            let data_filter = this.recordingList.filter(element => element.RecordingStatus.trim() == 'Finished');

            if (data_filter.length > 0) {
              this.recordingList = data_filter;
              this.checklist = this.recordingList;
  ...
  ReviewRecording(e, RefRecordingID, recordings) {
    ...
    localStorage.setItem('RefRecordingID', '574');
  ...
  getSelectedRowInfo(recordings, id, e) {
    ...
    if (r_id == id) {
      ...
      this.SharedTitle = recordings['AssignedTitle'];
