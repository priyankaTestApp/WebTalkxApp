...
@Component({
  selector: 'app-recordings-catalog-test',
  templateUrl: './recordings-catalog-test.component.html',
  styleUrls: ['./recordings-catalog-test.component.css']
})
export class RecordingsCatalogTestComponent implements OnInit {
  title: string;
  ...
  GetparticipantTypeRequestCount() {
    ...
    return this.http
      .get(`${environment.domainStageApi}FinalSubmission/GetParticipantTypeCount?UserID=`+ this.UserID+`&WebTalkxTypeName=Hiring&Status=Finished`).subscribe(res=>{
  ...
  GetReferenceRecording() {
    ...
    //showing loader
    this.appcmp.showLoader = true;
    this.appcmp.loadermessage = 'Loading hiring recordings...';
