...
@Component({
  selector: 'app-compliance-catalog',
  templateUrl: './compliance-catalog.component.html',
  styleUrls: ['./compliance-catalog.component.css']
})
export class ComplianceCatalogComponent implements OnInit {
  title: string;
  ...
  constructor(private http: HttpClient, public surveyCmp: SurveyApplicationComponent, private router: Router, private titleService: Title, private appcmp: AppComponent) {
    ...
    this.title = appcmp.title + " | Compliance Recordings";
  ...
  myFunction(data,index) {
      let urls: any = [], recordedby: any = [], titlearr: any = [],userID:any=[],emailID:any=[];
         ...
         let userId = data['UserID'] ||  data['GiverID'];
          ...
          console.log(email)
          localStorage.setItem('AssignedEmailID',email )
          if (recordingid) {
   ...
   this.tooltipVal =''
      if (document.queryCommandSupported && document.queryCommandSupported("copy")) {
        ...
        if (input.value) {
          ...
          // $('div[data-toggle="tooltip"]').attr("title","Secure URL copied");
  ...
  GetparticipantTypeRequestCount() {
    ...
    return this.http
      .get(`${environment.domainStageApi}FinalSubmission/GetParticipantTypeCount?UserID=`+ this.UserID+`&WebTalkxTypeName=Compliance&Status=Finished`).subscribe(res=>{
  ...
  GetReferenceRecording() {
    ...
    //showing loader
    this.appcmp.showLoader = true;
    this.appcmp.loadermessage = 'Loading compliance recordings...';
    ...
    //.post(`${environment.domainApi}I2BRecordingRequest/GetRecordingRequest`, reqObj).subscribe(res=>{
      .get(`${environment.domainStageApi}FinalSubmission/GetFinalSubmissionWebTalkxDetails?AuthorID=` +  this.UserID + `&WebTalkxTypeName=Compliance&Status=Finished`).subscribe(res=>{
      ...
      if (res["length"] > 0) {
        ...
        if (res[0]['ResponseStatus'] == 'Success') {
          ...
          let data_filter = this.RecordingRequestList.filter(element => (element.FinalStatus.trim() == 'Finished' && element.WebTalkxRecordingType.trim() == 'Single Recording'));
