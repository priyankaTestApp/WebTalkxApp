...
@Component({
  selector: 'app-security-catalog',
  templateUrl: './security-catalog.component.html',
  styleUrls: ['./security-catalog.component.css']
})
export class SecurityCatalogComponent implements OnInit {
  title: string;
  ...
  constructor(private http: HttpClient, public surveyCmp: SurveyApplicationComponent, private router: Router, private titleService: Title, private appcmp: AppComponent) {
    ...
    this.title = appcmp.title + " | Security Recordings";
  ...
  GetparticipantTypeRequestCount() {
    ...
    return this.http
      .get(`${environment.domainStageApi}FinalSubmission/GetParticipantTypeCount?UserID=`+ this.UserID+`&WebTalkxTypeName=Security&Status=Finished`).subscribe(res=>{
  ...
  GetReferenceRecording() {
    ...
    //showing loader
    this.appcmp.showLoader = true;
    this.appcmp.loadermessage = 'Loading security recordings...';
    ...
    //.post(`${environment.domainApi}I2BRecordingRequest/GetRecordingRequest`, reqObj).subscribe(res=>{
      .get(`${environment.domainStageApi}FinalSubmission/GetFinalSubmissionWebTalkxDetails?AuthorID=` +  this.UserID + `&WebTalkxTypeName=Security&Status=Finished`).subscribe(res=>{
