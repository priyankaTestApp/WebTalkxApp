...
export class RecordingsCatalogIDid2Component implements OnInit {
  ...
  isChecked: boolean;
  RequestTitle: string;
  webtakxtype:string = 'Offline Meeting';
  video_url:string;
  ...
  constructor(private http: HttpClient, public surveyCmp: SurveyApplicationComponent, private router: Router, private titleService: Title, private appcmp: AppComponent) {
    ...
    this.title = appcmp.title + " | Offline Meetings Recordings";
  ...
  ngOnInit() {
    ...
    this.model.WebtalkxType = 'Offline Meetings';
  ...
  GetparticipantTypeRequestCount() {
    ...
    return this.http
      .get(`${environment.domainStageApi}FinalSubmission/GetParticipantTypeCount?UserID=`+ this.UserID+`&WebTalkxTypeName=Offline Meeting&Status=Finished`).subscribe(res=>{
  ...
  GetReferenceRecording() {
    ...
    //showing loader
    this.appcmp.showLoader = true;
    this.appcmp.loadermessage = 'Loading offline meetings recordings...';
    ...
    //.post(`${environment.domainApi}I2BRecordingRequest/GetRequestedFromMe`, reqObj).subscribe(res=>{
      .get(`${environment.domainStageApi}FinalSubmission/GetFinalSubmissionWebTalkxDetails?AuthorID=` +  this.UserID + `&WebTalkxTypeName=Offline Meeting&Status=Finished`).subscribe(res=>{
