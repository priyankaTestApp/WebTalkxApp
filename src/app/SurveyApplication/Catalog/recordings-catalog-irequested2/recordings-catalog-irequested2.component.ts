...
@Component({
  selector: 'app-recordings-catalog-irequested2',
  templateUrl: './recordings-catalog-irequested2.component.html',
  styleUrls: ['./recordings-catalog-irequested2.component.css']
})
export class RecordingsCatalogIRequested2Component implements OnInit {
  title: string;
  ...
  constructor(private http: HttpClient, public surveyCmp: SurveyApplicationComponent, private router: Router, private titleService: Title, private appcmp: AppComponent) {
    ...
    this.title = appcmp.title + " | Reference Recordings";
  ...
  GetparticipantTypeRequestCount() {
    ...
    return this.http
      .get(`${environment.domainStageApi}FinalSubmission/GetParticipantTypeCount?UserID=`+ this.UserID+`&WebTalkxTypeName=Reference&Status=Finished`).subscribe(res=>{
      ...
      if (res["length"] > 0) {
        ...
        if (res[0]['ResponseStatus'] == 'Success') {
          ...
          if(this.single_count == 0) if(this.multiple_count > 0) this.model.ParticipantType = 'Multiple Participant';
  ...
  GetReferenceRecording() {
    ...
    //showing loader
    this.appcmp.showLoader = true;
    this.appcmp.loadermessage = 'Loading reference recordings...';

    let reqObj: any = {};
    reqObj.AuthorID = this.UserID;
    
    return this.http
    //.post(`${environment.domainApi}I2BRecordingRequest/GetRequestedFromMe`, reqObj).subscribe(res=>{
      .get(`${environment.domainStageApi}FinalSubmission/GetFinalSubmissionWebTalkxDetails?AuthorID=` +  this.UserID + `&WebTalkxTypeName=Reference&Status=Finished`).subscribe(res=>{
        ...
        if (res["length"] > 0) {
          ...
          if (res[0]['ResponseStatus'] == 'Success') {
            ...
            let data_filter = this.RecordingRequestList.filter(element => (element.FinalStatus.trim() == 'Finished' && element.ParticipantType.trim() == 'Single Recording'));
            let data_filter2 = this.RecordingRequestList.filter(element => (element.FinalStatus.trim() == 'Finished' && element.WebTalkxRecordingType.trim() == 'Group Recording'));
