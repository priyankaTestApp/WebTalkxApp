...
@Component({
  selector: 'app-vendor-management-catalog',
  templateUrl: './vendor-management-catalog.component.html',
  styleUrls: ['./vendor-management-catalog.component.css']
})
export class VendorManagementCatalogComponent implements OnInit {
  title: string;
  ...
  isChecked: boolean;
  RequestTitle: string;
  webtakxtype:string = 'Vendor Management';
  video_url:string;
  ...
  constructor(private http: HttpClient, public surveyCmp: SurveyApplicationComponent, private router: Router, private titleService: Title, private appcmp: AppComponent) {
    ...
    this.title = appcmp.title + " | Vendor Management Recordings";
  ...
  GetparticipantTypeRequestCount() {
    ...
    return this.http
      .get(`${environment.domainStageApi}FinalSubmission/GetParticipantTypeCount?UserID=`+ this.UserID+`&WebTalkxTypeName=Vendor Management&Status=Finished`).subscribe(res=>{
  ...
  GetReferenceRecording() {
    ...
    //showing loader
    this.appcmp.showLoader = true;
    this.appcmp.loadermessage = 'Loading vendor management recordings...';
    ...
    //.post(`${environment.domainApi}I2BRecordingRequest/GetRecordingRequest`, reqObj).subscribe(res=>{
      .get(`${environment.domainStageApi}FinalSubmission/GetFinalSubmissionWebTalkxDetails?AuthorID=` +  this.UserID + `&WebTalkxTypeName=Vendor Management&Status=Finished`).subscribe(res=>{
