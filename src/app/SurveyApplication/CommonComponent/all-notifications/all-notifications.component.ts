...
export class AllNotificationsComponent implements OnInit {
  //variables for sending data to survey application  
  dashboardData: any;
  @Output() notificationsData = new EventEmitter<string>();
  ...
  //method to get the all the notification (@Output used)
        ...
        .get(`${environment.domainApi}I2BNotifications/GetNotifications?UserID=` + uid + '&Count=3').subscribe(res => {
          ...
          if (res["length"] > 0) {
            ...
            if (res[0]['ResponseStatus'] == 'Success') {
              ...
              //we are emitting  data using @Output
              this.notificationsData.emit(this.notificationList);
  ...
  enableDisableNotificationIcon(unreaditemcount: number) {
    let notification_counter = document.getElementsByClassName('notification_counter')[0];

    if (unreaditemcount > 0) {
      notification_counter.innerHTML = String(unreaditemcount);
      if (notification_counter && notification_counter.classList.contains('-hidden')) notification_counter.classList.remove('-hidden');
    } else {
      if (notification_counter && !notification_counter.classList.contains('-hidden')) notification_counter.className += ' -hidden';
    }

  }

  enableDisableMessageIcon(unreaditemcount: number) {
    let message_counter = document.getElementsByClassName('message_counter')[0];

    if (unreaditemcount > 0) {
      message_counter.innerHTML = String(unreaditemcount);
      if (message_counter && message_counter.classList.contains('-hidden')) message_counter.classList.remove('-hidden');
    } else {
      if (message_counter && !message_counter.classList.contains('-hidden')) message_counter.className += ' -hidden';
    }
